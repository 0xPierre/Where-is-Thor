...
document.title = 'Trouvez Captain Marvel !'
...
const randomPositionImage = () => {
    ...
    pos_y.value = getRandomCoordBetween(backgroundImg.clientHeight / 1.5, backgroundImg.clientHeight - 100);
...
<template>
    <div class="container">
        <Header title="Trouvez Captain Marvel !" />
        <div class="background" @click="onClick">
            <img class="to-find" src="@/assets/images/levels/level3/tofind.png" ref="image" />
        ...
        <Modal :show="modalFailed" head-text="Dommage !" @on-close="modalFailed = false">
            <h3>Vous n'avez pas trouve Captain Marvel, reessayez !</h3>

            <template v-if="(trials % 3 === 0)">
                <p style="margin-top: 10px;">
                    Malheureusement Captain Marvel change d'endroit !
                </p>
            </template>
        ...
        <Modal :show="modalSuccess" head-text="Bien joue !">
            <h3>
                Bravo ! Tu as trouve Captain Marvel !
            </h3>
...
.background {
    height: 100%;
    width: 100%;
    background-image: url('@/assets/images/levels/level3/background.jpg');
